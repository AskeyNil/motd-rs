...
impl BetterCommand {
    ...
    pub fn new_with_bash() -> BetterCommand {
        let mut cmd = BetterCommand::new("bash");
        cmd.arg("-c");
        cmd
    }
...
mod tests {
    #[test]
    fn test_args() {
        let mut cmd = super::BetterCommand::new("echo");
        cmd.args(["Hello", "World"]);
        assert_eq!(cmd.get_output_string().unwrap(), "Hello World\n");
    }
}
